<template>
  <div class="flex animate-pulse" v-if="!proyects">
    <div class="mt-2 w-full">
      <ul class="mt-4 space-y-3">
        <li class="w-full mt-4 h-40 bg-gray-200 rounded-md dark:bg-gray-700"></li>
        <li class="w-full mt-4 h-40 bg-gray-200 rounded-md dark:bg-gray-700"></li>
      </ul>
    </div>
  </div>
  ...
  <div
    v-for="(proyectos, index) in proyects"
    :key="index"
    class="hover:shadow-lg my-4 flex flex-col bg-white border shadow-sm rounded-xl p-4 md:p-5 dark:bg-gray-800 dark:border-gray-700 dark:shadow-slate-700/[.7]"
  >
    <div class="inline-grid grid-cols-2 gap-4">
      ...
      <div class="relative">
        <div class="absolute top-0 right-0">
          <button
            type="button"
            v-on:click="deleteProyect(proyectos.id)"
...
export default defineComponent({
  ...
  methods: {
    ...
    showTaskProyect(proyecto: any) {
      this.$emit('showTask', proyecto)
    },
    async deleteProyect(id: any) {
      try {
        await axios.delete(`http://127.0.0.1:8000/api/proyect/${id}`, {
          headers: {
        ...
        this.$toast.open({
          message: `Error, no se puede eliminar el proyecto.`,
          type: 'error',
          position: 'bottom-right'
