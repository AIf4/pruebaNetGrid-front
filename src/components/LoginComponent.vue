<template>
    <div class="mx-auto my-auto max-w-screen-xl px-4 py-16 sm:px-6 lg:px-8 ">
        <div class="mx-auto max-w-lg text-center">
            <h1 class="text-2xl font-bold sm:text-3xl">Inicio de sesion</h1>
        ...
        <form action="" class="mx-auto mb-0 mt-8 max-w-md space-y-4" @submit.prevent="login">
            <div>
                <label for="email" class="sr-only">Correo electronico</label>

                <div class="relative">
                    <input type="email" class="w-full rounded-lg border-gray-200 p-4 pe-12 text-sm shadow-sm"
                        placeholder="Ingresa el email" v-model="email" />
            ...
            <div>
                ...
                <div class="relative">
                    <input type="password" class="w-full rounded-lg border-gray-200 p-4 pe-12 text-sm shadow-sm"
                        placeholder="Ingresa la contraseña" v-model="password" />
            ...
            <div class="flex items-center justify-between">
                <p class="text-sm text-gray-500">
                    No tienes cuenta?
                    <router-link to="/register" class="underline">Crear cuenta</router-link>
                </p>
    ...
    methods: {
        async login() {
            try {
                ...
                const {name, email, rol } = data.user;
                const user = {
                    name,
                    email,
                    isAdmin: rol.name === "admin" ? true : false
                }
